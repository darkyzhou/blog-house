...
export class WaterflowController {
  ...
  checkAndUpdateColumns(width) {
    const newColumnsCount = Math.min(Math.floor(width / this.preferredWidthPx), this.maxColumnsCount);
    if (!width || (newColumnsCount === this.columnsCount && !this.initial)) {
      return;
    }
    this.initial = false;
    const newColumns = Array(newColumnsCount).fill(null).map(() => []);
